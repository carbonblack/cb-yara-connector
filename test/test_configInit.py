...
class TestConfigurationInit(TestCase):
    ...
    def setUp(self):
        ...
        globals.g_utility_interval = -1
        globals.g_utility_script = None
    ...
    def test_17a_config_bogus_utility_interval(self):
        """
        Ensure that config with bogus (non-int) utility_interval is detected.
        """
        with self.assertRaises(ValueError) as err:
            ConfigurationInit(os.path.join(TESTS, "config", "bogus_utility_interval.conf"), "sample.json")
        assert "invalid literal for int" in "{0}".format(err.exception.args[0])

    def test_17b_config_negative_utility_interval(self):
        """
        Ensure that config with bogus (non-int) utility_interval is detected.
        """
        globals.g_utility_interval = None
        ConfigurationInit(os.path.join(TESTS, "config", "negative_utility_interval.conf"), "sample.json")
        self.assertEqual(0, globals.g_utility_interval)

    def test_18a_config_missing_utility_script(self):
        """
        Ensure that config with non-existing utility_script is detected.
        """
        with self.assertRaises(CbInvalidConfig) as err:
            ConfigurationInit(os.path.join(TESTS, "config", "no_such_utility_script.conf"), "sample.json")
        assert "does not exist" in "{0}".format(err.exception.args[0])

    def test_18b_config_bogus_utility_script_is_dir(self):
        """
        Ensure that config with utility_script as directory is detected.
        """
        with self.assertRaises(CbInvalidConfig) as err:
            ConfigurationInit(os.path.join(TESTS, "config", "utility_script_dir.conf"), "sample.json")
        assert "is a directory" in "{0}".format(err.exception.args[0])

    def test_18c_config_empty_utility_script(self):
        """
        Ensure that config with missing utility_script is detected.
        """
        ConfigurationInit(os.path.join(TESTS, "config", "empty_utility_script.conf"), "sample.json")
        self.assertEqual(0, globals.g_utility_interval)
        self.assertEqual("", globals.g_utility_script)

    def test_19a_config_utility_script_enabled(self):
        """
        Ensure that config with utility_script and utility_interval is ready to go.
        """
        globals.g_utility_interval = None
        globals.g_utility_script = None
        ConfigurationInit(os.path.join(TESTS, "config", "utility_script_enabled.conf"), "sample.json")
        self.assertEqual(360, globals.g_utility_interval)
        self.assertTrue(globals.g_utility_script.endswith("/scripts/vacuumscript.sh"))

    def test_19a_config_utility_script_and_no_utility_interval(self):
        """
        Ensure that config with utility_script but utility_interval == 0 has it disabled.
        """
        globals.g_utility_interval = None
        globals.g_utility_script = None
        ConfigurationInit(os.path.join(TESTS, "config", "utility_script_no_interval.conf"), "sample.json")
        self.assertEqual(0, globals.g_utility_interval)
        self.assertIsNone(globals.g_utility_script)
